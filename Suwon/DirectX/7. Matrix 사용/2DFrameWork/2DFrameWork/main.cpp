...
INT Init()
{
	...
	{
		MessageBox(NULL, "dx5_logo.bmp file Could not find", "Err", 0);
		m_pTx1 = NULL;
		return -1;
	}
...
INT Render()
{
	...
	m_pd3dDevice->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_NONE);//확대필터
	m_pd3dDevice->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_NONE);//축소필터
	m_pd3dDevice->SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_NONE);//밉맵 레벨간의 사용
	...
	mtW._11 = -.5f;
	mtW._22 = .5f;
	mtW._41 = (rt1.right - rt1.left) * 2.f;
	...
	mtW._42 = (rt1.bottom - rt1.top) * 2.f;
	m_pd3dSprite->SetTransform(&mtW);
	m_pd3dSprite->Draw(m_pTx1, &rt1, NULL, NULL, D3DXCOLOR(1, 1, 1, 1));
	...
	D3DXMATRIX	mtS;	//크기변환 행렬
	D3DXMATRIX	mtR;	//회전변환 행렬
	D3DXMATRIX	mtT1;	//이동변환 행렬
	D3DXMATRIX	mtT;	
	...
	D3DXMatrixScaling(&mtS, .8f, .8f, .8f);

	static float fAngle = 0;	fAngle += 0.1f;
	D3DXMatrixRotationZ(&mtR, fAngle);

	D3DXMatrixTranslation(&mtT, 400, 300, 0.f);
	D3DXMatrixTranslation(&mtT1, -128, -128, 0.f);
